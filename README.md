...
前端主要依赖 `public/data/*.json`。法学简报内容应由 AI 完成筛选、摘要和重要性分析；脚本只作为自动化执行器，负责调用 AI、保存结果和触发部署。每日简报默认新增 3 条法律新闻和 1 条法学研究；RSS 没有合格新条目时不会强行补旧内容。脚本会先清理既有低质量条目，再按更严格的法律相关性和编辑质量门槛追加新内容。
